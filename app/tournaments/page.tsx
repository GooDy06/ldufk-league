...
import { CalendarDays, Flag, Layers3, Monitor, UsersRound } from "lucide-react";
import {
  circuitConceptSteps,
  circuitLeagues,
  circuitSeason,
...
const leagueStyles = {
  school: {
    ...
    bg: "bg-school/10",
    glow: "shadow-[0_0_34px_rgba(110,231,183,0.08)]",
    ...
    <section className="soft-enter relative overflow-hidden rounded-2xl border border-line bg-surface sm:rounded-3xl">
      ...
      <div className="relative z-10 grid gap-3 p-3 sm:gap-5 sm:p-6 lg:grid-cols-[1fr_280px] lg:p-7">
        <div className="min-w-0">
          <h1 className="season-title-shine whitespace-nowrap font-rajdhani text-[clamp(1.26rem,5.55vw,1.65rem)] font-bold leading-none tracking-normal sm:text-4xl sm:leading-[0.95] sm:tracking-wide lg:text-5xl" data-title={circuitSeason.name}>
            {circuitSeason.name}
          </h1>
          ...
          <div className="mt-3 grid grid-cols-3 gap-1.5 sm:mt-4 sm:max-w-2xl sm:grid-cols-[1fr_1.12fr_0.72fr] sm:gap-2">
        ...
        <div className="grid grid-cols-3 content-start gap-1.5 sm:gap-2 lg:grid-cols-1 lg:self-start">
          <HeroMetric icon={CalendarDays} label="Season" value={circuitSeason.dates} mobileValue="10.26–05.27" />
          <HeroMetric icon={Layers3} label="Format" value={`${circuitSeason.leagues} · ${circuitSeason.onlineStages}`} mobileValue="2 ліги · 4 етапи" />
          <HeroMetric icon={Flag} label="Finals" value={circuitSeason.lanSlots} />
        </div>
      </div>
    </section>
  );
...
function HeroButton({
  ...
  const buttonClassName = `focus-ring flex min-w-0 items-center justify-center whitespace-nowrap rounded-lg border text-center font-extrabold uppercase transition ${sizeClass} ${className}`;

  return href.startsWith("#") ? (
    <SmoothAnchorLink href={href as `#${string}`} className={buttonClassName}>
      {children}
    </SmoothAnchorLink>
  ) : (
          ...
          {registrations.map(({ league, tournament }) => {
            ...
            return (
              <div key={tournament.slug} className="group rounded-lg border border-line bg-bg/45 p-2 transition duration-200 hover:-translate-y-0.5 hover:border-emerald-300/60 hover:bg-emerald-300/5 hover:shadow-[0_0_24px_rgba(52,211,153,0.12)] sm:rounded-xl sm:p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-x-2 gap-y-2 text-xs font-bold text-slate-300 sm:mt-3 sm:grid-cols-3 sm:gap-x-3">
                  <RegistrationFact icon={CalendarDays} label="Дата" value={tournament.month} />
                  <RegistrationFact icon={Monitor} label="Формат" value={tournament.format} />
                  <RegistrationFact icon={UsersRound} label="Склад" value="Опція 4+1" className="hidden sm:flex" />
                  <span className="prize-sparkle col-span-2 flex flex-col items-center justify-center rounded-lg border border-gold/50 bg-[radial-gradient(circle_at_50%_0%,rgba(250,204,21,0.26),transparent_48%),linear-gradient(135deg,rgba(250,204,21,0.18),rgba(250,204,21,0.07)_48%,rgba(7,10,16,0.78))] px-3 py-3 text-center text-gold shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_0_26px_rgba(250,204,21,0.16)] backdrop-blur-md transition group-hover:border-gold/75 group-hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_0_34px_rgba(250,204,21,0.22)] sm:col-span-3">
...
function RegistrationFact({ icon: Icon, label, value, className = "" }: { icon: typeof CalendarDays; label: string; value: string; className?: string }) {
  return (
    <div className={`flex min-w-0 items-center gap-1.5 border-l border-white/10 pl-1.5 sm:gap-2 sm:pl-2 ${className}`}>
      <Icon className="h-3.5 w-3.5 shrink-0 text-emerald-200/80 sm:h-4 sm:w-4" strokeWidth={1.8} />
      <div className="min-w-0">
        <div className="text-[7px] font-extrabold uppercase tracking-[0.16em] text-slate-500">{label}</div>
        <div className="text-[9px] font-bold leading-tight text-slate-300 sm:text-[11px]">{value}</div>
      </div>
    </div>
  );
}

function HeroMetric({ icon: Icon, label, value, mobileValue }: { icon: typeof CalendarDays; label: string; value: string; mobileValue?: string }) {
  return (
    <div className="flex items-center gap-1.5 border-l border-white/15 pl-2 sm:gap-2.5 sm:pl-3">
      <Icon className="h-3.5 w-3.5 shrink-0 text-accent/75 sm:h-4 sm:w-4" strokeWidth={1.8} />
      <div className="min-w-0">
        <div className="text-[7px] font-extrabold uppercase tracking-[0.16em] text-slate-500 sm:text-[9px] sm:tracking-[0.2em]">{label}</div>
        <div className="mt-0.5 truncate font-rajdhani text-[11px] font-bold leading-tight text-slate-100 sm:text-lg">
    ...
    <section className="soft-enter grid gap-2 rounded-xl border border-line bg-surface p-2 sm:gap-4 sm:rounded-2xl sm:p-4 lg:grid-cols-[190px_1fr]">
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-bg/35">
        {circuitConceptSteps.map((step, index) => (
          <div key={step.title} className="grid grid-cols-[28px_82px_1fr] items-start gap-1.5 border-b border-line px-2 py-2 last:border-b-0 sm:grid-cols-[54px_150px_1fr] sm:items-center sm:gap-2 sm:px-3 sm:py-2.5">
            ...
            <p className="text-[9px] leading-3.5 text-slate-400 sm:text-[13px] sm:leading-5">{step.text}</p>
          </div>
        ))}
      </div>
    </section>
...
function RoadToFinals() {
  return (
    <section id="road-to-finals" className="soft-enter scroll-mt-28 rounded-xl border border-line bg-surface p-2 sm:rounded-2xl sm:p-5">
      <div className="mb-2 flex flex-col justify-between gap-2 sm:mb-4 sm:flex-row sm:items-end sm:gap-3">
        ...
        <p className="max-w-2xl text-[10px] leading-4 text-slate-400 sm:text-sm sm:leading-6">
      ...
      <div className="overflow-x-auto pb-1">
        <div className="relative min-w-[930px] px-2 pt-1">
          <div className="absolute left-[7.4%] right-[7.4%] top-[24px] h-px bg-slate-700" />
        ...
        {roadToFinals.map((stage, index) => {
          ...
          return (
          <div key={stage.title} className={`relative flex min-h-[214px] flex-col pt-10 ${isLocked ? "text-slate-400 saturate-[0.68]" : ""}`}>
            <span className={`absolute left-1/2 top-0 z-10 grid h-12 w-12 -translate-x-1/2 place-items-center rounded-full border-2 bg-bg font-rajdhani text-base font-bold ${
              isActive
                ? "border-school text-school shadow-[0_0_22px_rgba(110,231,183,0.35)]"
            ...
            <div className={`flex flex-1 flex-col rounded-lg border p-2.5 ${
              isActive
                ? "border-school/45 bg-school/[0.06]"
                : "border-slate-700/70 bg-slate-900/55"
            }`}>
              <span className={`mb-2 w-fit whitespace-nowrap rounded-full border px-1.5 py-1 text-[7px] font-extrabold uppercase leading-none tracking-[0.08em] ${isActive ? "border-school/35 text-school" : `${roadStageStyles[stage.kind].border} ${roadStageStyles[stage.kind].text}`}`}>{roadStageBadge[stage.kind]}</span>
              <h3 className={`font-rajdhani text-xl font-bold leading-none ${isLocked ? "text-slate-300" : ""}`}>{stage.title}</h3>
              <p className="mt-1.5 flex-1 text-[10px] leading-4 text-slate-400">{stage.description}</p>
              <div className={`${isLocked ? "" : "prize-sparkle"} mt-2 flex min-h-[56px] flex-col justify-center rounded-md border px-2 py-1.5 ${isLocked ? "border-slate-600/55 bg-slate-800/45 text-slate-300" : "border-gold/45 bg-[linear-gradient(135deg,rgba(250,204,21,0.18),rgba(250,204,21,0.07)_48%,rgba(13,20,32,0.92))] shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_0_22px_rgba(250,204,21,0.1)]"}`}>
              ...
              <div className={`mt-0.5 font-rajdhani text-lg font-bold leading-none ${isLocked ? "text-slate-300" : "text-gold"}`}>{stage.prizePool}</div>
              {stage.kind === "points" ? (
    ...
    <article className={`interactive-card group flex flex-col overflow-hidden rounded-lg border sm:rounded-xl lg:min-h-[250px] ${isFinals ? "col-span-2 lg:col-span-1" : ""} ${
      ...
      <div className={`border-b border-line p-2 sm:p-2.5 ${isRegistration ? "bg-emerald-400/10" : "bg-slate-800/20"}`}>
        <div className="flex items-center justify-between gap-1">
          <span className="text-[7px] font-extrabold uppercase tracking-[0.14em] text-slate-500 sm:text-[8px]">Stage {index + 1}</span>
          <span className={`shrink-0 whitespace-nowrap rounded-full border px-1.5 py-0.5 text-[7px] font-extrabold uppercase leading-none tracking-[0.06em] sm:py-1 sm:text-[8px] sm:tracking-[0.08em] ${isRegistration ? "border-emerald-300/35 bg-emerald-300/15 text-emerald-200" : isFinals ? "border-slate-500/25 bg-slate-500/10 text-slate-400" : "border-orange-300/35 bg-orange-300/10 text-orange-200"}`}>{isFinals ? "Locked" : tournament.status}</span>
        </div>
        <h3 className={`mt-1.5 min-h-[40px] line-clamp-2 font-rajdhani text-base font-bold leading-tight sm:mt-2 sm:text-lg lg:min-h-[42px] ${isUnavailable ? "text-slate-300" : ""}`}>{tournament.name}</h3>
        <div className="mt-0.5 truncate text-[7px] font-bold uppercase tracking-[0.12em] text-slate-500 sm:mt-1 sm:text-[9px] sm:tracking-[0.14em]">{tournament.role}</div>
      </div>

      <div className="flex flex-1 flex-col p-2 sm:p-2.5">
        <div className="grid gap-1.5 text-[10px] sm:gap-2">
          <TournamentFact icon={CalendarDays} label="Місяць" value={tournament.month} />
          <TournamentFact icon={Monitor} label="Формат" value={tournament.format} />
        </div>
        ...
        <div className="mt-2 space-y-1.5 sm:mt-2.5 sm:space-y-2">
          ...
          <div className="hidden text-[11px] font-semibold leading-4 text-slate-500 sm:block">
            {tournament.points ? tournament.entry : "Визначає чемпіона сезону."}
          </div>
          {isRegistration ? (
            ...
            <span aria-disabled="true" className="block rounded-md border border-slate-700 bg-slate-900/70 px-2 py-1.5 text-center text-[9px] font-extrabold uppercase tracking-[0.08em] text-slate-500 sm:rounded-lg sm:px-3 sm:py-2 sm:text-xs sm:tracking-[0.12em]">
              {isFinals ? "Очікується" : "Зареєструватися"}
            </span>
          )}
        </div>
      </div>
...
function TournamentFact({ icon: Icon, label, value }: { icon: typeof CalendarDays; label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center gap-1.5 border-l border-white/10 pl-1.5 sm:gap-2 sm:pl-2">
      <Icon className="h-3.5 w-3.5 shrink-0 text-accent/75" strokeWidth={1.8} />
      <div className="min-w-0">
        <div className="text-[7px] font-extrabold uppercase tracking-[0.12em] text-slate-500 sm:text-[8px] sm:tracking-[0.16em]">{label}</div>
        <div className="text-[10px] font-semibold leading-tight text-slate-300 sm:text-xs">{value}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <section className="soft-enter rounded-xl border border-line bg-surface p-2 sm:rounded-2xl sm:p-5">
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 sm:mt-4 sm:gap-3">
        {qualificationSlots.map((slot, index) => {
          const Icon = [Flag, Layers3, UsersRound][index];
          return (
          <div key={slot.title} className="flex min-w-0 gap-1.5 border-l border-accent/25 pl-1.5 sm:gap-2.5 sm:pl-3">
            <Icon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-accent sm:h-4 sm:w-4" strokeWidth={1.8} />
            <div className="min-w-0">
              <div className="text-[7px] font-extrabold uppercase tracking-[0.1em] text-accent sm:text-[10px] sm:tracking-[0.16em]">{slot.slots}</div>
              <h3 className="mt-0.5 font-rajdhani text-[13px] font-bold leading-tight sm:text-lg">{slot.title}</h3>
              <p className="mt-0.5 text-[8px] leading-3 text-slate-400 sm:text-xs sm:leading-5">{slot.description}</p>
            </div>
          </div>
          );
        })}
      ...
      <div className="mt-3 grid grid-cols-3 gap-1.5 border-t border-line pt-2 sm:mt-4 sm:gap-2 sm:pt-3">
        {qualificationZones.map(([range, label]) => (
          <div key={range} className="min-w-0 px-1 sm:px-2">
            <div className="font-rajdhani text-base font-bold text-slate-100 sm:text-xl">{range}</div>
...
function CircuitCta() {
  return (
    <section className="soft-enter rounded-xl border border-accent/25 bg-[linear-gradient(135deg,rgba(0,213,255,0.12),rgba(110,231,183,0.06),rgba(13,20,32,0.92))] p-2 sm:rounded-2xl sm:p-5">
      <div className="flex flex-col justify-between gap-2 sm:gap-4 md:flex-row md:items-center">
        ...
        <div className="grid grid-cols-3 gap-1.5 sm:flex sm:flex-wrap sm:gap-2">
          <HeroButton href="/ranking" size="xs">Рейтинг</HeroButton>
