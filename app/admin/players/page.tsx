...
function PlayerForm({ player, teams }: { player?: Player; teams: Team[] }) {
  return (
    <form action={savePlayer} className="grid gap-3 rounded-2xl border border-line bg-surface p-4">
      ...
      <button className="w-fit rounded-lg bg-accent px-4 py-2 font-bold text-bg">{player ? "Зберегти" : "Створити"}</button>
      {player ? <button formAction={deletePlayer} name="id" value={player.id} className="w-fit rounded-lg border border-red-500/30 px-3 py-2 text-sm font-bold text-red-300">Видалити</button> : null}
    </form>
  );
}
