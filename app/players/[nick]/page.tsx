  ...
  return (
    <div className="py-8">
      <section className="overflow-hidden rounded-2xl border border-line bg-surface">
        <div className="grid gap-5 bg-surface2 p-5 md:grid-cols-[260px_1fr]">
          <div className="relative min-h-[260px] overflow-hidden rounded-xl border border-line bg-gradient-to-br from-slate-700/40 to-bg">
          ...
          <div>
            ...
            <h1 className="font-rajdhani text-6xl font-bold text-slate-100">{displayNick}</h1>
            <div className="mt-2 text-xl font-bold uppercase tracking-widest text-slate-500">{player?.role || "Rifler"}</div>
            <div className="mt-5 divide-y divide-line text-lg">
              <div className="grid grid-cols-[180px_1fr] py-3"><strong className="text-slate-400">Current team</strong><span>{teamName}</span></div>
              <div className="grid grid-cols-[180px_1fr] py-3"><strong className="text-slate-400">Rating 3.0</strong><span className={rating >= 1 ? "text-emerald-300" : "text-red-300"}>{rating.toFixed(2)}</span></div>
              <div className="grid grid-cols-[180px_1fr] py-3"><strong className="text-slate-400">Maps</strong><span>{stats?.maps || 0}</span></div>
              <div className="grid grid-cols-[180px_1fr] py-3"><strong className="text-slate-400">K/D</strong><span>{stats ? `${stats.kills}/${stats.deaths} (${stats.kd.toFixed(2)})` : "0/0"}</span></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-2xl border border-line bg-surface p-4">
          <div className="mb-3 flex items-end justify-between">
            <h2 className="font-rajdhani text-3xl font-bold">{displayNick} statistics</h2>
