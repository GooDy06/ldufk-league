...
export function RankingList({ teams }: { teams: RankingTeam[] }) {
  ...
  return (
    <div className="grid min-w-[860px] gap-3 md:min-w-0">
      {teams.map((team, index) => {
        ...
        return (
          <article key={team.id} className={`overflow-hidden rounded-2xl border bg-surface2 transition ${isOpen ? "border-accent/45 shadow-[0_0_30px_rgba(0,213,255,0.08)]" : "border-line hover:border-accent/25"}`}>
            <button type="button" onClick={() => toggleTeam(team.id)} className="grid w-full grid-cols-[52px_46px_1fr_auto_auto] items-center gap-3 p-3 text-left">
              ...
              <div className={`text-sm font-bold ${team.trend > 0 ? "text-emerald-300" : team.trend < 0 ? "text-red-300" : "text-slate-500"}`}>{team.trend === 0 ? "" : trendLabel}</div>
