...
function renderParagraph(block: string, index: number) {
  ...
  if (imageMatch && isSafeUrl(imageMatch[2])) {
    return (
      <figure key={index} className="my-8 overflow-hidden rounded-2xl border border-line bg-surface">
        <img src={publicImageUrl(imageMatch[2])} alt={imageMatch[1]} loading="lazy" decoding="async" className="max-h-[520px] w-full object-cover" />
  ...
  if (isSafeUrl(rawImageUrl) && isImageUrl(rawImageUrl)) {
    return (
      <figure key={index} className="my-8 overflow-hidden rounded-2xl border border-line bg-surface">
        <img src={publicImageUrl(rawImageUrl)} alt="" loading="lazy" decoding="async" className="max-h-[520px] w-full object-cover" />
      </figure>
    );
  }
