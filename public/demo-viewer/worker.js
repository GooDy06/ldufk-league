const WASM_BASE_URL = "/demo-viewer/";
...
async function loadWasm() {
  ...
  const response = await fetch(WASM_BASE_URL + "wasm/csdemoparser.wasm");
  const bytes = await response.arrayBuffer();
  await WebAssembly.instantiate(bytes, go.importObject).then((result) => {
